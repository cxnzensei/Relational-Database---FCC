...
echo "$($PSQL "select distinct(name) from games left join teams on teams.team_id=games.winner_id where round='Eighth-Final' and year=2014 union select distinct(name) from games left join teams on teams.team_id=games.opponent_id where year=2014 and round='Eighth-Final' order by name")"
